Hashroot Core version *0.15.1* is now available from:

  <https://download.hashroot.org/hashroot-0.15.1/>
...
A number of changes to the way Hashroot Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 stws per kB (0.00001 STW). This is to prepare for dropping the minimum transaction fee to 0.1 stws per kB (0.0001 STW) in the next release.
