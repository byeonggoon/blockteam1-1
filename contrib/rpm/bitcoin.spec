...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9708
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9701
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19708
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19701
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 9708
	%{_sbindir}/semanage port -d -p tcp 9701
	%{_sbindir}/semanage port -d -p tcp 19708
	%{_sbindir}/semanage port -d -p tcp 19701
	for selinuxvariant in %{selinux_variants}; do
